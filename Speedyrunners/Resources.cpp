...
Resources::Resources()
{
	...
	while (!file.eof()) {
		...
		else if (type == "A" || type == "A2" || type == "AM" || type == "AW" || type == "AS") { // Audio (soundbuffers)
			std::string path = PATH_TO_PLAYER_SFX;
			if (type == "A2") { // Ingame ui audio
				path = PATH_TO_INGAME_UI_SFX;
			}
			else if (type == "AW") // Weapons
				path = PATH_TO_ITEMS_SFX;

			else if (type == "AM") // Menus
				path = PATH_TO_MENU_SFX;
			else if (type == "AS") // Stingers 
				path = PATH_TO_STINGERS_SFX;
			std::string line;
